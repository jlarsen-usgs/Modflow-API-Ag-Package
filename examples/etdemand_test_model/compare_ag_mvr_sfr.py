...
def build_mf6(name, headtol=None, fluxtol=None):
    sim_ws = os.path.join(sws, "..", "..", "data", "mf6_etdemand_test_problems", name)
...
def compare_model_output(nwt, mf6, model):
    ...
    with styles.USGSPlot():
        ...
        ax.plot(range(1, len(mf6_div1) + 1), mf6_div1.q_from_provider.values, color="skyblue",  label=f"mf6 diversion 1", ls="--", lw=2.5, zorder=3)
        ax.plot(range(1, len(mf6_div2) + 1), mf6_div2.q_from_provider.values, color="darkblue", label=f"mf6 diversion 2", ls="--", lw=2.5, zorder=3)
