...
def build_model(name, sim_ws):
    ...
    for i in range(nrow):
        for j in range(ncol):
            ...
            if cnt in bound_cells:
                rec = (cnt, (0, i, j), 1, 0, 1.0, vks, 0.2, 0.38, 0.2, 7.5, "ag_irr")
            else:
    ...
    obs_dict = {
        "uzf_obs.csv": [
            ("gwrch", "uzf-gwrch", "ag_irr"),
            ("uzet", "uzet", "ag_irr"),
            ("gwet", "uzf-gwet", "ag_irr"),
            ("storage", "storage", "ag_irr"),
            ("net_infiltration", "net-infiltration", "ag_irr"),
            ("infiltration", "infiltration", "ag_irr")
    ...
    for per in range(nper):
        ...
        if per <= 3:
            ...
            for cell in uzfcells:
                rec = ("sfr_0", sfr_reach, "uzf_0", cell, div_p_cell, 1.0, app_fact)
                spd.append(rec)

            for ix, cell in enumerate(uzfcells):
                rec = ("wel_0", ix, "uzf_0", cell, ag_pump, 1.0, app_fact)
                spd.append(rec)
...
if __name__ == "__main__":
    names = ["prudic_base", "prudic_af_1.1", "prudic_af_0.9"]
    for name in names:
        sim_ws = os.path.join(
            "..", "..", "data", "mf6_prudic_ag_scenarios", name
        )
