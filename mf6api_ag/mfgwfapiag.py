...
# flopy.mf6.utils.create_api_packages.create_api_package
# FILE created on September 07, 2022 20:10:19 UTC
...
class ModflowGwfapiag(mfpackage.MFPackage):
    ...
    print_input : boolean
        * print_input (boolean) description REPLACE print_input {'{#1}':
          'AGMVR'}
    print_flows : boolean
        * print_flows (boolean) description REPLACE print_flows {'{#1}':
          'AGMVR'}
    modelnames : boolean
    ...
    agmethod : string
        * agmethod (string) optional agricultural simulation method, currently
          supported options are "ETDEMAND", defaults to "ETDEMAND"
    maxmvr : integer
    ...
    packages : [mname, pname]
        ...
        * pname (string) is the name of a package that may be included in a
          ...
          so forth.
    perioddata : [mname1, pname1, id1, mname2, pname2, id2, value, irr_eff,
      app_frac]
        ...
        * irr_eff (double) efficiency of irrigation method for calculating
          applied water to the soil zone. Value must be between 0 and 1.
        * app_frac (double) scale factor for irrigation calculation to allow
          for deficit irrigation or flush irrigation. Values between 0 and 1
          indicate deficit irrigation and values greater than 1 will simulate
          flush irrigation.
    filename : String
    ...
    """
    budget_filerecord = ListTemplateGenerator(('gwf6', 'agmvr',
                                               'options',
                                               'budget_filerecord'))
    budgetcsv_filerecord = ListTemplateGenerator(('gwf6', 'agmvr',
                                                  'options',
                                                  'budgetcsv_filerecord'))
    packages = ListTemplateGenerator(('gwf6', 'agmvr', 'packages',
                                      'packages'))
    perioddata = ListTemplateGenerator(('gwf6', 'agmvr', 'period',
                                        'perioddata'))
    ...
    dfn = [
           ...
           ["block options", "name budgetcsvfile", "type string",
            "preserve_case true", "shape", "in_record true", "reader urword",
            "tagged false", "optional false"],
           ["block options", "name agmethod", "type string",
            "reader urword", "optional true"],
           ...
           ["block period", "name perioddata",
            "type recarray mname1 pname1 id1 mname2 pname2 id2 value irr_eff "
            "app_frac",
           ...
           ["block period", "name value", "type double precision", "shape",
            "tagged false", "in_record true", "reader urword"],
           ["block period", "name irr_eff", "type double precision",
            "shape", "tagged false", "in_record true", "reader urword"],
           ["block period", "name app_frac", "type double precision",
            "shape", "tagged false", "in_record true", "reader urword"]]
    ...
    def __init__(self, parent_model_or_package, loading_package=False,
                 ...
                 agmethod=None, maxmvr=None, maxpackages=None, packages=None,
                 perioddata=None, filename=None, pname=None, **kwargs):
        super().__init__(parent_model_or_package, "agmvr", filename, pname,
        ...
        self.budgetcsv_filerecord = self.build_mfdata("budgetcsv_filerecord",
                                                      budgetcsv_filerecord)
        self.agmethod = self.build_mfdata("agmethod", agmethod)
