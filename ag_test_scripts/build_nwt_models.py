...
def build_nwt_well_test(name):
    model_ws = os.path.join(sws, "..", "data", "nwt_etdemand_test_problems")
...
def build_nwt_diversion_test(name):
    model_ws = os.path.join(sws, "..", "data", "nwt_etdemand_test_problems")
    ...
    for per in range(12):
        ...
        divflow = 10
        segment_data[10] = (
            11, 1, 0, 3, 0, 0, divflow, 0, 0, 0, 0.03, 0, 0, 0, 0, 0, 0.000015,
    ...
    gage = flopy.modflow.ModflowGage(
        ml,
        numgage=2,
        gage_data=[(11, 1, 55, 0), (12, 1, 56, 5)],
        files=["gage11.txt", "gage12.txt"]
    )
...
if __name__== "__main__":
    ...
    if not load_existing:
        ml = build_nwt_well_test("etdemand_well")
        ml2 = build_nwt_diversion_test("etdemand_div")
    else:
        model_ws = os.path.join(sws, "..", "data", "nwt_etdemand_test_problems")
        ...
        ml2 = flopy.modflow.Modflow.load(
            "etdemand_diversion.nam", model_ws=model_ws, version="mfnwt"
        )
