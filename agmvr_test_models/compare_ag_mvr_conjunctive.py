...
def build_mf6(name, headtol=None, fluxtol=None):
    sim_ws = os.path.join(sws, "..", "data", "mf6_agmvr_test_problems")
    ...
    for i in range(12):
        ...
        for col in range(23, 25):
            sfrrec = ("sfr_0", 2, "uzf_0", col, 10., 1.0, 1.1)
            wellrec = ("wel_0", 0, "uzf_0", col, 25, 1.0, 1.1)
            mvr_rec.append(sfrrec)
            mvr_rec.append(wellrec)
        for col in range(43, 45):
            sfrrec = ("sfr_0", 4, "uzf_0", col, 5., 0.9, 1.0)
            wellrec = ("wel_0", 1, "uzf_0", col, 20, 0.9, 1.0)
            mvr_rec.append(sfrrec)
            mvr_rec.append(wellrec)
...
def compare_model_output(nwt, mf6, model):
    ...
    total_ag2 = mf6_div2.q_to_receiver.values + mf6_well2.q_to_receiver.values
    ...
    with styles.USGSPlot():
        ...
        ax.plot(range(1, len(mf6_div1) + 1), mf6_div1.q_from_provider, color="skyblue", label=f"mf6 diversion", ls="--", lw=2.5, zorder=5)
    ...
    with styles.USGSPlot():
        ...
        ax.plot(range(1, len(mf6_div2) + 1), mf6_div2.q_to_receiver, color="skyblue",
